      ...
      {editing && draft && (
        <div style={card}>
          ...
          <label style={{ display: 'flex', alignItems: 'center', gap: 8, margin: '12px 0 4px', fontSize: 13, cursor: 'pointer' }}>
            ...
            Es tipo <strong>Demo / Gratuito</strong>
          </label>
          {draft.isDemo && (
            <div style={{ marginTop: 8, padding: 12, background: 'var(--bg3)', borderRadius: 8 }}>
              <div style={{ fontWeight: 700, fontSize: 13, marginBottom: 2 }}>🎁 Plan Gratuito</div>
              <p style={{ fontSize: 11.5, color: 'var(--text2)', margin: '0 0 10px' }}>
                Las cuentas gratuitas/demo tienen acceso <strong>completo</strong> a la plataforma (todos los módulos y la IA).
                Su única limitación son las <strong>conversaciones del mes</strong>: al alcanzarlas la IA deja de responder
                hasta que se renueva el ciclo o el cliente contrata un plan.
              </p>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(180px,1fr))', gap: 12 }}>
                <Field label="Conversaciones incluidas / mes"><input type="number" min="1" style={inp} value={draft.demoMaxConversations ?? 100} onChange={e => set('demoMaxConversations', e.target.value)} /></Field>
              </div>
              <details style={{ marginTop: 10 }}>
                <summary style={{ fontSize: 12, color: 'var(--text3)', cursor: 'pointer' }}>Ajustes legacy (Demo de 7 días · solo cuentas antiguas)</summary>
        ...
        <div key={t.id} style={{ ...card, display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap' }}>
          <div style={{ flex: 1, minWidth: 200 }}>
            ...
            <div style={{ fontSize: 12, color: 'var(--text2)', marginTop: 4 }}>
              ...
              {t.isDemo && <span style={{ color: 'var(--amber)' }}> · 🎁 {t.demoMaxConversations || 100} conversaciones/mes</span>}
