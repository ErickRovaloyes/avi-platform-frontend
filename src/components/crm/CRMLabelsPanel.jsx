...
export default function CRMLabelsPanel() {
  const { account, addLabel, deleteLabel } = useAccount()
  ...
  function handleAdd(e) {
    ...
    addLabel({ name: nLabel.name.trim(), color: nLabel.color })
  ...
  return (
    <div style={card}>
      ...
      <div style={{ fontSize: 12, color: 'var(--text2)', marginBottom: 12 }}>
        Crea etiquetas para clasificar conversaciones y contactos. Se usan en el Inbox y en los filtros del CRM.
      </div>

      <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 14 }}>
        {labels.map(l => (
          <div key={l.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 10px', background: 'var(--bg3)', border: '1px solid var(--border2)', borderRadius: 8 }}>
