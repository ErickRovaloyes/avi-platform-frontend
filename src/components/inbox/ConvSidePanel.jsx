...
import PipelineCardModal from '../pipeline/PipelineCardModal'
...
// ── Pestaña 🎫 Tickets: lista las tarjetas (deals) del contacto; al hacer clic en una
// se abre el MISMO popup del pipeline (PipelineCardModal) con TODAS las opciones.
function TicketsTab({ conv, agentId }) {
  const { account } = useAccount()
  const pipelines = account?.pipelines || []
  const [modalCard, setModalCard] = useState(null)   // { pipeId, cardId }
  const [showAdd, setShowAdd] = useState(false)

  // Tarjetas vinculadas a la conversación (resueltas contra los pipelines de la cuenta).
  const convCards = (conv.pipelineCards || []).map(pc => {
    ...
    return { card, pipe, stageName: stage?.name, stageColor: stage?.color }
  }).filter(Boolean)

  // Resuelve pipe+card en vivo (para que el popup refleje las ediciones guardadas).
  const openPipe = modalCard && pipelines.find(p => p.id === modalCard.pipeId)
  const openCard = openPipe && openPipe.cards?.find(c => c.id === modalCard.cardId)

  return (
    <div className={s.section}>
      <div className={s.sTitle}>Tickets</div>
      {pipelines.length === 0
        ? <div className={s.empty}>No tienes pipelines. Créalos en la pestaña CRM.</div>
        : <>
          {convCards.length === 0 && <div className={s.empty} style={{ marginBottom: 10 }}>Este chat no tiene tickets todavía.</div>}
          {convCards.map(({ card, pipe, stageName, stageColor }) => (
            <div key={card.id} className={s.pipeCard} style={{ cursor: 'pointer' }} title="Abrir el ticket con todas las opciones"
              onClick={() => setModalCard({ pipeId: pipe.id, cardId: card.id })}>
              <div className={s.pipeCardTop}>
                <span className={s.pipeCardTitle}>🧲 {card.title}</span>
                <span className={s.pipeCardPipe}>{pipe.name}</span>
              </div>
              {stageName && (
                <span className={s.pipeCardStage} style={{ background: stageColor + '22', color: stageColor, borderColor: stageColor + '55' }}>
                  {stageName}
                </span>
              )}
              {card.value ? <span className={s.pipeCardValue}>${card.value}</span> : null}
            </div>
          ))}
          <button onClick={() => setShowAdd(true)}
            style={{ marginTop: 6, width: '100%', padding: '9px 12px', borderRadius: 8, border: '1px dashed var(--border2)', background: 'transparent', color: 'var(--text2)', cursor: 'pointer', fontSize: 12.5, fontWeight: 600 }}>
            + Agregar a un pipeline
          </button>
        </>}

      {openPipe && openCard && (
        <PipelineCardModal key={openCard.id} pipe={openPipe} card={openCard} onClose={() => setModalCard(null)}
          onOpenCard={(pipeId, cardId) => {
            ...
            setModalCard({ pipeId, cardId })
          }} />
      )}
      {showAdd && <PipelineConvoModal conv={conv} agentId={agentId} onClose={() => setShowAdd(false)} />}
    </div>
  )
}
